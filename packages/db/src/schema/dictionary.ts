...
// never exampleSentence or contexts, since a sentence drawn from a book can
// leak plot. Storing the columns here is fine; selecting them in that one view
// is not. (The generated sentences are invented rather than quoted — the
// enrichment prompt forbids referencing a plot, character, or setting — but
// the query constraint stands on its own rather than trusting that.)
export const dictionaryEntry = pgTable(
  "dictionary_entry",
  {
    ...
    // Invented sentences showing the term in use, each with the term itself
    // wrapped in braces: "The house had an {eldritch} stillness about it."
    // See `parseContext` in @better-vocab/domain — the marked span is what
    // lets one stored string be both a context panel entry and a cloze quiz
    // card, without either reader guessing where the word sits.
    //
    // An array rather than a table of its own: two or three short strings,
    // always read with the row and never queried individually, so a join would
    // buy nothing. Same shape as `book.authors`.
    //
    // Shared and term-keyed like everything else here, which is what lets the
    // quiz cost nothing: the cards a reader is shown were generated by the one
    // enrichment call this term will ever get.
