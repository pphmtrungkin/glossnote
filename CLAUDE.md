...
- **No bundled `core` dictionary asset ships.** `scripts/build-dictionary-db.ts` and `wordnet-to-dictionary.ts` can build one, but no `.db` asset is committed and `SQLiteProvider` passes no `assetSource`, so a device with no downloaded pack starts with an empty `dictionary` table and offline lookups fall through. A built asset stamps `user_version = 3` itself so `migrateLocalDb` leaves it alone — keep the two in sync.
- ~~**The `extended` tier has no way in.**~~ — built. `lib/dictionary-pack.ts` downloads the pack (`expo-file-system`'s `File.createDownloadTask`, with progress) from `EXPO_PUBLIC_DICTIONARY_PACK_URL` and merges it with SQLite's own `ATTACH` + one `INSERT OR IGNORE ... SELECT`, never row-by-row in JS. The pack is built with `source = 'extended'` already stamped, which is what makes the column list line up; the `(term, source, rank)` unique index is what makes a re-run idempotent. Removing it is `DELETE FROM dictionary WHERE source = 'extended'` — the reason the three tier tables were collapsed into one in `0002` of the local schema. Install size and row count are recorded in `app_setting` because `count(*) where source = ...` cannot use that index and would scan the table. `app/settings.tsx` drives it: installing also sets `preference.offlineDictionaryTier` to `extended`, since the *pack* is per-device and the *preference* is per-account.
- ~~**`user_preference` has no router.**~~ — fixed. `preference.get`/`preference.update` exist, `get` returns `PREFERENCE_DEFAULTS` without writing a row, and `update` upserts field by field. `app/settings.tsx` is the caller; it also hosts sign-out, which used to be a tab-header button of its own.
