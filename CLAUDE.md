...
- **Screens** (`apps/native/app/`) — `(tabs)/index` shelf, `(tabs)/search`, `folder/[id]`, `add-word`, `(auth)/sign-in`. Page themes live in `contexts/app-theme-context.tsx`: daisyUI's **`autumn`** (the default) plus four Kindle page colours.
