...
- `apps/server/.env` — `DATABASE_URL`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `CORS_ORIGIN`, `PORT`, and the optional `HARDCOVER_API_TOKEN` and `ANTHROPIC_API_KEY` (schema: `packages/env/src/server.ts`). Set `SKIP_ENV_VALIDATION=true` to bypass.

`ANTHROPIC_API_KEY` powers AI enrichment and is **server-only by SoftwareSpec §8.1** — the Expo app calls its own tRPC procedures, never a provider — so it must never become an `EXPO_PUBLIC_*` var either. Optional: without it, `dictionary.lookup` still resolves and caches base definitions. Model choice lives in `enrich.ts` alone; the spec asks for a small/fast tier (§8.4) because an example sentence is not frontier-reasoning work.
